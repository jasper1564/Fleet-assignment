...
if __name__ == "__main__":
    ...
    if model.Status == GRB.OPTIMAL:
        ...
        all_legs = data_tuple[3]  # 新增：获取所有的航段列表
        ...
        # 由于原模型包含整数变量（机型分配 x 为 Binary），直接提取 .Pi 会报错。
        # 这里使用 model.fixed() 创建一个固定了最优整数解的连续松弛模型 (LP)
        fixed_model = model.fixed()
        fixed_model.setParam('OutputFlag', 0) # 关闭多余输出
