...
def build_and_solve_fap_model(
    ...
    for l in all_legs:
        ...
        model.addConstr(demand_on_leg <= capacity_of_leg, name=f"leg_capacity[{l}]")
...
if __name__ == "__main__":
    ...
    if model.Status == GRB.OPTIMAL:
        ...
        for p in products:
            ...
            if sold > 0.01: 
                ...
                comprehensive_records.append({
                    ...
                    "Unmet_Demand": round(demand[p] - sold, 2),
                    "Legs": " -> ".join(p_legs),
                    "Tasks": " -> ".join(p_tasks),
        ...
        with open('result_comprehensive_analysis.csv', 'w', newline='', encoding='utf-8-sig') as f_out:
            fieldnames = ["Product", "Max_Demand", "Sold_Tickets", "Unmet_Demand", "Legs", "Tasks", "Assigned_Fleets"]
        ...
        # ==========================================
        # 导出 3：航段影子价格 (Bid Price)
        # ==========================================
        print("🔍 正在计算网络航段影子价格(Bid Prices)...")
        ...
        if fixed_model.Status == GRB.OPTIMAL:
            ...
            for l in all_legs:
                constr = fixed_model.getConstrByName(f"leg_capacity[{l}]")
                if constr:
                    pi_val = constr.Pi
                    shadow_price_records.append({
                        "Leg": l,
                        "Shadow_Price": round(pi_val, 2)
                    })
            with open('result_shadow_prices.csv', 'w', newline='', encoding='utf-8-sig') as f_out:
                writer = csv.DictWriter(f_out, fieldnames=["Leg", "Shadow_Price"])
                writer.writeheader()
        ...
        for l in all_legs:
            ...
            total_passengers_on_leg = sum(s[p].X for p in leg_to_products.get(l, []))
            load_factor = (total_passengers_on_leg / assigned_seats * 100) if assigned_seats > 0 else 0
            
            suspicious = "Yes" if load_factor < 99.9 and any(demand[p] > s[p].X + 0.01 for p in leg_to_products.get(l, [])) else "No"
            ...
            leg_load_records.append({
                ...
                "Total_Passengers": round(total_passengers_on_leg, 2),
                "Load_Factor(%)": round(load_factor, 2),
                "Bottleneck_or_Cost_Constrained": suspicious
            })
        ...
        with open('result_leg_load_factor.csv', 'w', newline='', encoding='utf-8-sig') as f_out:
            fieldnames = ["Leg", "Parent_Task", "Assigned_Fleet", "Seat_Capacity", 
                          "Total_Passengers", "Load_Factor(%)", "Bottleneck_or_Cost_Constrained"]
            writer = csv.DictWriter(f_out, fieldnames=fieldnames)
            writer.writeheader()
        ...
        print("   - result_shadow_prices.csv          (各航段的影子价格/网络竞价表)")
        print("   - result_leg_load_factor.csv        (航段物理容量与客座率探针)")
