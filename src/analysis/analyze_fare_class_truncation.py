...
FARE_LEVEL_TRUNCATION_OUTPUT = ROOT / "data" / "interim" / "truncation" / "fare_level_truncation_flags.csv"
FARE_RATE_FIGURE = ROOT / "results" / "figures" / "truncation" / "fare_truncation_rate.png"
FARE_RD_FIGURE = ROOT / "results" / "figures" / "truncation" / "fare_last_sale_rd_distribution.png"
FARE_BOOKING_FIGURE = ROOT / "results" / "figures" / "truncation" / "fare_booking_curves.png"
...
def analyze_fare_level_truncation(input_filepath, output_filepath, zero_days_threshold=7):
    ...
    plt.tight_layout()
    FARE_RATE_FIGURE.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(FARE_RATE_FIGURE, dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(FARE_RD_FIGURE, dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(FARE_BOOKING_FIGURE, dpi=300)
    plt.close()

    print("图表生成完毕！重点观察 fare_plot_2 寻找关舱策略证据。")

if __name__ == "__main__":
    analyze_fare_level_truncation(input_filepath=RAW_PRODUCTS_INPUT, 
                                  output_filepath=FARE_LEVEL_TRUNCATION_OUTPUT,
