...
def analyze_carrier_truncation(input_filepath, output_filepath, zero_days_threshold=7):
    ...
    plt.tight_layout()
    plt.savefig('carrier_plot_1_truncation_rate.png', dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('carrier_plot_2_death_time_dist.png', dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('carrier_plot_3_booking_curves.png', dpi=300)
    plt.close()
    
    print("生成完毕！重点关注 carrier_plot_1 和 carrier_plot_2 寻找数据丢失证据。")

if __name__ == "__main__":
    analyze_carrier_truncation(input_filepath="data_fam_products.csv", 
                               output_filepath="data_fam_flights_carrier_analyzed.csv", 
