...
UNCONSTRAINED_DEMAND_INPUT = ROOT / "data" / "raw" / "demand" / "itinerary_unconstrained_demand.csv"
EM_PRODUCTS_OUTPUT = ROOT / "data" / "interim" / "demand_recovery" / "products_with_em_demand.csv"
EM_PRODUCT_INFO_OUTPUT = ROOT / "data" / "model_input" / "demand" / "product_info_em_restored.json"

def update_demand():
    # 1. 读取两个 CSV 文件
    print("正在读取数据文件...")
    try:
        df_products = pd.read_csv(PROCESSED_PRODUCTS_INPUT)
        df_fam = pd.read_csv(UNCONSTRAINED_DEMAND_INPUT)
    except FileNotFoundError as e:
        print(f"文件读取失败，请检查文件路径和名称是否正确: {e}")
        return

    print(f"{PROCESSED_PRODUCTS_INPUT.name} 行数: {len(df_products)}")
    print(f"{UNCONSTRAINED_DEMAND_INPUT.name} 行数: {len(df_fam)}")
    ...
    # 3. 保存为新的 CSV 文件
    output_file = EM_PRODUCTS_OUTPUT
    ...
    try:
        ...
        # 写入 JSON 文件
        json_output_file = EM_PRODUCT_INFO_OUTPUT
