...
def load_real_data(data_dir="."):
    ...
    with open(os.path.join(data_dir, 'product_info.json'), 'r', encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    if model.Status == GRB.OPTIMAL:
        ...
        for f in tasks:
            for k in fleets:
                if x[f, k].X > 0.5:
                    assignment_records.append({"Task": f, "Assigned_Fleet": k})
        
        with open('result_assignment.csv', 'w', newline='', encoding='utf-8-sig') as f_out:
            writer = csv.DictWriter(f_out, fieldnames=["Task", "Assigned_Fleet"])
            writer.writeheader()
            writer.writerows(assignment_records)
        ...
        with open('result_sales.csv', 'w', newline='', encoding='utf-8-sig') as f_out:
            writer = csv.DictWriter(f_out, fieldnames=["Product", "Sold_Tickets", "Max_Demand"])
            writer.writeheader()
        ...
        print("   - result_assignment.csv     (航班机型分配明细)")
        print("   - result_sales.csv          (产品实际销量明细)")
