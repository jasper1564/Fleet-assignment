...
try:
    df_schedule = pd.read_csv('data_fam_schedule.csv')
    df_products = pd.read_csv('data_fam_products.csv')
...
df_schedule.to_csv('processed_schedule.csv', index=False, encoding='utf-8-sig')
df_products.to_csv('processed_products.csv', index=False, encoding='utf-8-sig')
df_mapping.to_csv('processed_mapping.csv', index=False, encoding='utf-8-sig')

print("\n🎉 数据清洗全部完成！请在当前文件夹下查看以下 3 个新文件：")
print("1. processed_schedule.csv (包含 Duration, Overnight, Super_Flight_ID 等)")
print("2. processed_products.csv (包含 ProductID, Total_Demand 等)")
print("3. processed_mapping.csv  (产品与航段的长表映射关系)")
